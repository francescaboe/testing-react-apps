...
the best way to test, is by mocking the user behaviour.
To test any component we first render it in the dom:
- create a div
- append div to body
- use the div as root (createRoot)
- then render our component within that root

Once we have the component rendered
We grab whatever ui element (eg a button(s): div.querySelectorAll('button')), 
and we simulate an action using the act(()=>actionHere) for example click
[the dispatchEvent api allows to simulate other events with no dedicated dom method eg mouseover]
then we grab any other elements we need (eg a div containing some text)
then we use expect assertion to check that the message is what we expect it to be after the action we fired.

And finally we need to clean up the dom
