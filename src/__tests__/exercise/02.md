...
RTL provides a bunch of abstractions that allow us to skip some boilerplate:
so instead of:
...
use createRoot to render the component-to-be-tested to the container div
```const root = createRoot(div);```

wrap everything in act and render
```act(() => root.render(<Counter />))```


we can use the rtl abstraction render

where container === div container
```const {container} = render(<MyComponent/>)```
and it also wraps everything inside act, which is a jest requirement


we also get some other utilities like 

```fireEvent.whateverEventWeNeed (eg mouseEnter/mouseClick)```
