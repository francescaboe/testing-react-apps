...
following the principle of testing as a user would use the software, we should write our test based on elements that
are unrelated with the specific implementation of a certain solution.
ie grab a button based on their role and label rather than rely on the order or the number of buttons for example.
we can use scree and queries like getByText.

useEvent mimics more closely the user behaviour, for example for click, it will also add mouseenter, mouseover etc

https://testing-playground.com/ >> to get the type of query to use
